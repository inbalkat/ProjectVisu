...
# Calculate yearly slope for salaries
def calculate_salary_slopes(df):
    df["slope"] = df["salary"].diff() / df["year"].diff()
    return df

salary_df = calculate_salary_slopes(salary_df)

# Calculate predicted prices based on salary slopes
def calculate_predicted_prices(real_prices, salary_slopes):
    predicted_prices = [real_prices.iloc[0]]
    for i in range(1, len(real_prices)):
        predicted = predicted_prices[-1] + salary_slopes.iloc[i] * (real_prices.index[i] - real_prices.index[i - 1])
...
def prepare_data(real_df, salary_df, value_column):
    ...
    salary_slopes = salary_df.set_index("year")["slope"].fillna(0)
    predicted_prices = calculate_predicted_prices(real_prices, salary_slopes)
